...
HRESULT FireWall::IsFireWallOn(OUT PBOOL isOn)
{
	/*! 방화벽이 켜져 있는지 확인. */
	FireWall::FireWallImpl fireWall;
	return fireWall.IsWindowsFirewallOn(*isOn);
}

HRESULT FireWall::IsFireWallContainApp(IN LPCTSTR szProcessImageName, OUT PBOOL isOn)
{
	/*! 프로그램이 방화벽 목록에 등록되어 있는지 확인. */
	FireWall::FireWallImpl fireWall;
	return fireWall.IsAppEnabled(szProcessImageName, *isOn);
}

HRESULT FireWall::AddFireWallApp(IN LPCTSTR szProcessImageName, IN LPCTSTR policyName)
{
	/*! 방화벽 목록에 프로그램 등록. */
	FireWall::FireWallImpl fireWall;
	return fireWall.AddApplication(szProcessImageName, policyName);
}

HRESULT FireWall::TurnFireWallOn()
{
	/*! 방화벽 on. */
	FireWall::FireWallImpl fireWall;
	return fireWall.TurnOffWindowsFirewall();
}

HRESULT FireWall::TurnFireWallOff()
{
	/*! 방화벽 off. */
	FireWall::FireWallImpl fireWall;
	return fireWall.TurnOffWindowsFirewall();
...
namespace Common
{
	namespace FireWall
	{
		FireWallImpl::FireWallImpl(void)
		{
		...
		FW_ERROR_CODE FireWallImpl::Initialize()
		{
			HRESULT hr = S_FALSE;
		...
		FW_ERROR_CODE FireWallImpl::Uninitialize()
		{
			...
			if (SUCCEEDED(m_comInit)) CoUninitialize();

			return FW_NOERROR;
		}

		FW_ERROR_CODE FireWallImpl::IsWindowsFirewallOn(BOOL& bOn)
		{
			HRESULT hr;
		...
		FW_ERROR_CODE FireWallImpl::TurnOnWindowsFirewall()
		{
			HRESULT hr;
		...
		FW_ERROR_CODE FireWallImpl::TurnOffWindowsFirewall()
		{
			HRESULT hr;
			try
			{
		...
		FW_ERROR_CODE FireWallImpl::IsAppEnabled(LPCTSTR lpszProcessImageFileName, BOOL& bEnable)
		{
			FW_ERROR_CODE ret = FW_NOERROR;
			HRESULT hr;
		...
		FW_ERROR_CODE FireWallImpl::AddApplication(LPCTSTR lpszProcessImageFileName, LPCTSTR lpszRegisterName)
		{
			FW_ERROR_CODE ret = FW_NOERROR;
			HRESULT hr;
			BOOL bAppEnable;
		...
		FW_ERROR_CODE FireWallImpl::RemoveApplication(LPCTSTR lpszProcessImageFileName)
		{
			FW_ERROR_CODE ret = FW_NOERROR;
			HRESULT hr;
			BOOL bAppEnable;
		...
		FW_ERROR_CODE FireWallImpl::IsPortEnabled(LONG lPortNumber, NET_FW_IP_PROTOCOL ipProtocol, BOOL& bEnable)
		{
			FW_ERROR_CODE ret = FW_NOERROR;
			VARIANT_BOOL bFWEnabled;
		...
		FW_ERROR_CODE FireWallImpl::AddPorts(LONG lPortNumber, NET_FW_IP_PROTOCOL ipProtocol, LPCTSTR lpszRegisterName)
		{
			FW_ERROR_CODE ret = FW_NOERROR;
			INetFwOpenPort* pFWOpenPort = NULL;
			INetFwOpenPorts* pFWOpenPorts = NULL;
		...
		FW_ERROR_CODE FireWallImpl::RemovePort(LONG lPortNumber, NET_FW_IP_PROTOCOL ipProtocol)
		{
			FW_ERROR_CODE ret = FW_NOERROR;
			INetFwOpenPorts* pFWOpenPorts = NULL;
			HRESULT hr;
		...
		FW_ERROR_CODE FireWallImpl::IsExceptionNotAllowed(BOOL& bNotAllowed)
		{
			FW_ERROR_CODE ret = FW_NOERROR;
		...
		FW_ERROR_CODE FireWallImpl::SetExceptionNotAllowed(BOOL bNotAllowed)
		{
			FW_ERROR_CODE ret = FW_NOERROR;
		...
		FW_ERROR_CODE FireWallImpl::IsNotificationDiabled(BOOL& bDisabled)
		{
			FW_ERROR_CODE ret = FW_NOERROR;
		...
		FW_ERROR_CODE FireWallImpl::SetNotificationDiabled(BOOL bDisabled)
		{
			FW_ERROR_CODE ret = FW_NOERROR;
		...
		FW_ERROR_CODE FireWallImpl::IsUnicastResponsesToMulticastBroadcastDisabled(BOOL& bDisabled)
		{
			FW_ERROR_CODE ret = FW_NOERROR;
		...
		FW_ERROR_CODE FireWallImpl::SetUnicastResponsesToMulticastBroadcastDisabled(BOOL bDisabled)
		{
			FW_ERROR_CODE ret = FW_NOERROR;
